<template>
  <div :id="'stockLineItem'+kineType" class="kcandle_div">
    ...
    <div v-for="item in items" class="move-item" :key="item">
      <div
        :id="'tooltipId'+item.type+kineType"
        :charttype="item.type"
        :class="'tooltips w-100 text-left tooltopStyle'+item.type"
      ></div>
        ...
        this_.$api.get(url, null, rs => {
          ...
          for (let j = 0; j < chartArray.length; j++) {
            ...
            switch (chartType) {
              ...
              case 2:
                chartObj.setOption(
                  stockVol(this_.$utils.clone(srcDatas), this_.kineType)
                );
                break;
              case 3:
                chartObj.setOption(
                  stockRsi(this_.$utils.clone(srcDatas), this_.kineType)
                );
                break;
              case 4:
                chartObj.setOption(
                  stockDmi(this_.$utils.clone(srcDatas), this_.kineType)
                );
                break;
              case 5:
                chartObj.setOption(
                  stockMacd(this_.$utils.clone(srcDatas), this_.kineType)
                );
                break;
              case 6:
                chartObj.setOption(
                  stockTower(this_.$utils.clone(srcDatas), this_.kineType)
                );
                break;
            }
            // var index = 50; //播放所在下标
