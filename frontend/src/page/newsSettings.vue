<template>
  <main-layout>
    <div class="border border-primary m-2 float-left" style="width: 45%">
      ...
      <div id="add-exclude-item" class="w-3 float-left p-1"></div>
    ...
    <div class="border border-success m-2 float-left" style="width: 45%">
      ...
      <div id="add-include-item" class="w-3 float-left p-1"></div>
      <div class="clearfix"></div>
    </div>
    <div class="clearfix"></div>
  </main-layout>
...
export default {
  ...
  methods: {
    saveExcludeItem(event) {
      ...
      if (values.length > 0) {
        let url =
          "/api/stock/saveStockNewsKeyByExclude?keys[]=" + values.join();
        let $this = this;
    ...
    saveIncludeItem(event) {
      ...
      if (values.length > 0) {
        let url =
          "/api/stock/saveStockNewsKeyByInclude?keys[]=" + values.join();
        let $this = this;
        this.$api.post(url).then(rs => {
          this.getIncludesItems();
        });
