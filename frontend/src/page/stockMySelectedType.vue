<template>
  <div>
    <span>
      ...
      <b-button
        :variant="currSelectedType=='' || currSelectedType=='0'?'warning':'success'"
        @click="getAllStockMyStore"
      >庫存股</b-button>
    </span>
    <span :key="item" v-for="item in items">
      <b-button
        ...
        :id="item.type"
        @click="getMyStockSelected(item.type, item.name)"
      >
        {{ item.name }}
    ...
    <!-- Modal Component -->
    <b-modal id="modalPrevent2" ref="modal" title="请输入自選股名称" @ok="handleOk" @shown="clearName">
...
export default {
  ...
  created() {
    ...
    Bus.$on(
      ...
      (selectedType, myStockSelectedName) => {
