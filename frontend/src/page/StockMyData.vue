...
export default {
  ...
  created() {
    ...
    Bus.$on("getMySubStockSelected", (id, name) => {
      this.myStockSelectedName = name;
      this.$api.get("/api/stock/getStockMyDatasBySubId/" + id).then(r => {
        this.list = r;
        if (r != undefined && r.length > 0) {
          this.firstStockId = r[0].stockId;
          //改变路由的地址
    ...
    Bus.$on("getAllStockMyStore", () => {
      ...
      // this.myStockSelectedName = name
      this.$api.get("/api/stock/getStockMyDatasByStore").then(r => {
        this.list = r;
        if (r != undefined && r.length > 0) {
          this.firstStockId = r[0].stockId;
          // let stockId = this.$route.params.stockId == undefined ? this.firstStockId : this.$route.params.stockId
  ...
  methods: {
    ...
    isSelected(_stockId) {
      let stockId =
        this.$route.params.stockId == undefined
          ? this.firstStockId
    ...
    getData() {
      this.$api.get("/api/stock/getStockMyDatasByStore").then(r => {
        this.list = r;
        if (
          this.$route.params.stockId != null &&
          this.$route.params.stockId.length > 10
        ) {
          // 不是正常的id长度
    ...
    getStockMySelectedTypes() {
      ...
      if (stockId != undefined && stockId != "") {
        this.$api
          .get("/api/stock/getMySelectedTypesByStockId/" + stockId)
          .then(rs => {
            let mySelectedType = [];
